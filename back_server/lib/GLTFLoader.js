...
class GLTFBinaryExtension {
    constructor(data) {
        ...
        const chunkContentsLength = this.header.length -
            BINARY_EXTENSION_HEADER_LENGTH;
        ...
        while (chunkIndex < chunkContentsLength) {
            ...
            if (chunkLength > chunkView.byteLength - chunkIndex) {
                console.error(
                    'Chunk length exceeds remaining data length:',
                    chunkLength,
                );
                throw new Error(
                    'THREE.GLTFLoader: Chunk length exceeds remaining data length.',
                );
            }
...
class GLTFDracoMeshCompressionExtension {
    ...
    decodePrimitive(primitive, parser) {
        ...
        for (const attributeName in gltfAttributeMap) {
            const threeAttributeName = ATTRIBUTES[attributeName] ||
...
class GLTFParser {
        ...
        return Promise.all(pendingBufferViews).then(function (bufferViews) {
            ...
            if (byteStride && byteStride !== itemBytes) {
                ...
                const ibCacheKey = 'InterleavedBuffer:' +
                    ...
                    accessorDef.componentType + ':' + ibSlice + ':' +
                    accessorDef.count;
            ...
            if (accessorDef.sparse !== undefined) {
                ...
                const byteOffsetValues = accessorDef.sparse.values.byteOffset ||
                    0;
    ...
    loadTextureImage(textureIndex, source, loader) {
        ...
        if (source.bufferView !== undefined) {
                ...
                .then(
                    function (bufferView) {
                        if (source.mimeType === 'image/png') {
                            ...
                            const colorType = new DataView(bufferView, 25, 1)
                                .getUint8(
                                    0,
                                    false,
                                );
            ...
            throw new Error(
                'THREE.GLTFLoader: Image ' + textureIndex +
    ...
    assignFinalMaterial(mesh) {
        ...
        const useMorphTargets =
            Object.keys(geometry.morphAttributes).length > 0;
        const useMorphNormals = useMorphTargets &&
        ...
        if (
            useVertexTangents || useVertexColors || useFlatShading ||
            useSkinning ||
            useMorphTargets
        ) {
    ...
    loadMaterial(materialIndex) {
        ...
        if (
            ...
            materialType = this._invokeOne(function (ext) {
                return ext.getMaterialType &&
                    ext.getMaterialType(materialIndex);
            });
...
function addPrimitiveAttributes(geometry, primitiveDef, parser) {
    ...
    function assignAttributeAccessor(accessorIndex, attributeName) {
        return parser.getDependency('accessor', accessorIndex).then(
            function (accessor) {
    ...
    if (primitiveDef.indices !== undefined && !geometry.index) {
        const accessor = parser.getDependency('accessor', primitiveDef.indices)
